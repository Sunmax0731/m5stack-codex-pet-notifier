...
Codex Pets の素材を使う場合は、firmware build / upload の前に local asset header を生成します。生成先の `firmware/include/pet_asset.local.h` は `.gitignore` 対象で、release asset に含めません。既定では scale `1..8` ごとの Core2 用高解像度 frame も同じ header に含めます。
