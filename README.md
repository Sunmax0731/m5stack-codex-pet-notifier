...
- Domain: IoT
- Idea No: 18
- Runtime gate: simulator / mock device / sample telemetry / host adapter / LAN Host Bridge / long-run diagnostics / Core2 soak runner / Codex relay / Codex app-server runtime probe / device adapter / installer signing pipeline readiness / security boundary
...
- `installer/wix/` と `installer/msix/` に署名付き MSI / MSIX 化のテンプレートを配置し、`npm run installer:signed:pipeline` で WiX source / MSIX payload、release 環境では署名と verify まで進める。
- `src/codex-adapter/appServerAdapter.mjs` は Codex App Server の public interface を使うための JSON-RPC message builder、安全な transport gate、timeout 付き stdio session を提供する。非公開 API scraping は adapter review で禁止する。
...
`npm test` は `docs/platform-runtime-gate.json`、`dist/validation-result.json`、`docs/qcds-regression-baseline.json`、`dist/m5stack-codex-pet-notifier-docs.zip` を生成または更新します。Core2 実機の長時間運用 evidence は `npm run core2:soak` が `dist/core2-soak-result.json` と `docs/core2-soak-result.json` へ保存します。
...
Codex Pets の素材を使う場合は、firmware build / upload の前に local asset header を生成します。生成先の `firmware/include/pet_asset.local.h` は `.gitignore` 対象で、release asset に含めません。既定では hatch-pet 標準 atlas の 9 行（idle / running-right / running-left / waving / jumping / failed / waiting / running / review）を取り込み、Core2 用には idle の全 frame と各 row の代表 pose を scale `1..8` の高解像度 frame set として同じ header に含めます。
...
- [manual-test-automation.md](docs/manual-test-automation.md)
