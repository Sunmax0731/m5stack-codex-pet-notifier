...
- Domain: IoT
- Idea No: 18
- Runtime gate: simulator / mock device / sample telemetry / host adapter / LAN Host Bridge / long-run diagnostics / Codex relay / Codex app-server runtime probe / device adapter / installer signing pipeline readiness / security boundary
...
この release は simulator、mock device、LAN Host Bridge smoke、Codex relay smoke、Codex app-server adapter smoke / runtime probe、adapter review、Dashboard smoke、Core2 firmware build、署名 pipeline readiness を検証対象にした beta prerelease です。GRAY 実機と GRAY IMU は対象外です。実 Codex App Server 接続は `turn/start` まで確認済みです。長時間運用と実署名付き MSI / MSIX は release 環境での formal gate として残します。複数 M5Stack 同時接続は今回対象外で、今後のアップデート対象です。
