...
- `M5Stack 表示プレビュー` で Core2 / GRAY、local hatch-pet asset、pet background RGBA、text color RGBA、text background RGBA、Codex answer beep を変更できる。
- `M5Stack 表示プレビュー` は画面プレビューと設定フォームが2カラムで並び、同じ設定量でも以前より縦方向の占有が少ない。
- 状態確認、送信、プレビュー、最近の Codex 回答、イベントログ、デバッグが2カラムで並び、状態確認、イベントログ、デバッグだけが全幅を占有していない。
