...
- `text background` を変えると Answer / Decision / Notification の本文パネルと footer の全ての文字背景に反映される。pet display area が `6/8` 以上の fullscreen layout でも本文パネルが text background 色で描かれ、screen background に同期しない。
