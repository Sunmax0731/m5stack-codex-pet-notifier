...
- 10 秒程度待った後の inbound `device.heartbeat` details に `display.applyCount` と `display.lastEventId` が出る。`display.petScale`、`display.petOffsetX/Y`、`display.screenBackgroundRgba`、`display.petBackgroundRgba`、`display.textBackgroundRgba` が送信した値と一致する。
