...
| --- | --- | --- | --- |
| LR-01 長時間 soak | 準備中 | Host Bridge は heartbeat age、stale、droppedEvents を JSON で確認可能 | Core2 実機を 8時間以上接続し続ける環境 |
| LR-02 Wi-Fi AP停止 / 復帰 | 準備中 | firmware 側 backoff / pairing 復帰は source gate と実機 heartbeat で確認可能 | AP停止 / 復帰操作、Core2 serial log |
