...
| Area | Grade | Reason |
| --- | --- | --- |
| Quality | S- | schema、LAN Host Bridge smoke、Codex relay smoke、simulator、representative suite、runtime gate、security/privacy、Core2 upload / Wi-Fi / pairing / Codex answer 表示が検証対象。GRAY 実機と長時間運用は未確認のため S+ ではない |
...
| Satisfaction | S- | 導入、操作、Host Bridge 手動確認、Codex relay 手順、残範囲が明確。長時間 UX 未確認のため S+ ではない |
...
Core2 の upload、Wi-Fi、pairing、Codex relay answer、A button reply は確認対象です。GRAY 実機、長時間運用、実 Codex App 内部 API 連携は対象外です。残範囲があるため `S+` は付けません。
