...
| Area | Grade | Reason |
| --- | --- | --- |
| Quality | S- | schema、LAN Host Bridge smoke、Dashboard smoke、Dashboard 最新 Codex 回答表示/送信 smoke、Decision request smoke、pet mood / gesture smoke、long press から Choice request への side effect、current pet preview smoke、Display `1..32` / render FPS / motion step 設定 smoke、M5Stack 表示プレビュー smoke、Codex relay smoke、Codex session smoke、Codex hook relay smoke、Codex app-server adapter smoke、Codex app-server runtime probe、adapter review、clipboard UTF-8 relay smoke、simulator、representative suite、runtime gate、security/privacy、Core2 upload / Wi-Fi / pairing / Codex answer 表示、日本語フォント対応、8時間 Core2 soak、long-run source gate、signing readiness、signed installer pipeline、hatch-pet asset gate、scale-specific pet frame source gate、pet render FPS / motion step / display scale source gate、M5Canvas Sprite buffer による pet-only redraw source gate、M5Stack Choice Gate 配布 docs が検証対象。実署名付き MSI / MSIX は未確認のため S+ ではない |
...
- `cmd.exe /d /s /c npm run formal:automation -- --skip-npm-test --include-turn --skip-wifi-interruption`
