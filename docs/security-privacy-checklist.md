...
| Codex App 内部 API に固定依存しない | Pass | Codex relay は clipboard / stdin / file を source にし、非公開 API scraping を行わない |
...
- token、host IP、実会話本文、個人 pet sprite、`firmware/include/pet_asset.local.h`、`.env` は含めない。
- clipboard / file watch で扱う実 Codex 返答本文は release evidence に本文として保存しない。
