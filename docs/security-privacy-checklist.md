...
| Codex App 内部 API に固定依存しない | Pass | Codex relay は clipboard / stdin / file / local session JSONL を source にし、Codex App Server adapter は public interface だけを対象にする。非公開 API scraping を行わない |
...
| Codex App Server WebSocket を不用意に公開しない | Pass | adapter は stdio 既定。WebSocket は loopback または auth 必須として検証する |
...
| Display 設定に個人情報を含めない | Pass | `display.settings_updated` は petScale、uiTextScale、bodyTextScale、animationFps、motionStepMs、screen / pet / text / border RGBA、petOffsetX/Y、textBorderEnabled、beepOnAnswer、visualProbe の設定値だけを扱う |
...
| 署名証明書を保存しない | Pass | signing readiness は env 名だけを確認し、PFX や password を repo / release asset に保存しない |
