...
Host Bridge の beta endpoint は LAN 内限定です。公開ネットワークやポート開放した環境では起動しません。
...
- `M5Stack 表示プレビュー` から pet display area を `1..32`、UI text size と body text size を `1..8`、render FPS を `4..20`、motion step を `120..800ms` で送信し、M5Stack 上の pet 表示面積、文字サイズ、描画更新上限、キャラ frame 切替頻度を調整する。
- `M5Stack 表示プレビュー` の `device` で Core2 / Button reference を切り替え、同じ 320x240 画面で入力前提と footer label の違いを確認する。Button reference は IMU なしの preview で、GRAY 実機 target ではありません。
...
- pet mood を `idle / listening / thinking / happy / surprised / confused / sleepy / worried / alert / proud` から選び、`ペット更新を送信` で実機の表情または姿勢を切り替える。local hatch-pet asset では図形 marker ではなくキャラクターイラストの row が変わります。
- Core2 の pet を single tap / double tap / long press / swipe すると、Dashboard event log と preview readout に `device.pet_interacted` が表示されます。long press は Codex 側に三択判断を求める `prompt.choice_requested` を自動 queue します。
...
実 Codex App 公開 API 連携へ進める場合は、まず adapter smoke と adapter review を実行します。
...
- pet surface は hatch-pet asset が生成済みならその素材を表示し、未生成なら fallback avatar を表示する。Core2 では display area `1..32` に応じて scale-specific frame を選び、必要に応じて追加拡大する。local hatch-pet asset の透明ピクセルは固定アクセント背景ではなく、pet background RGBA の合成色を見せます。
- local hatch-pet asset の表情 / 姿勢は atlas row で切り替わります。`thinking` は running、`listening` は waiting、`surprised` / `reacting` は waving、`happy` / `proud` は jumping、`confused` / `review` は review、`alert` / `worried` / `sleepy` / `failed` は failed row を使います。tap / double tap / long press / swipe でも短時間 row が切り替わります。
...
- pet display area は Dashboard の `M5Stack 表示プレビュー` から `1..32` を切り替えられる。`8/32` は pet を画面全体に近く表示し、`32/32` は超拡大で頭や一部だけを見せる構図調整に使える。
...
## Button Reference Preview

- Dashboard preview では Core2 と同じ 320x240 画面で button 前提の footer label を確認できます。
- IMU は扱いません。
- firmware build、flash、実機検証の対象ではありません。
