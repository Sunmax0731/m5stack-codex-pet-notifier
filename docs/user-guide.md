...
- `環境構築コマンド` modal の `デバッグ送信` tab、または modal 内の直接送信フォームから Codex の返答本文を M5Stack へ送る。
- `環境構築コマンド` modal の `デバッグ送信` tab、または modal 内の直接送信フォームから A/B/C の確認依頼を送り、M5Stack で押された返信を inbound と modal の `Decision 返信` で確認する。
...
- sidebar の `環境構築コマンド` からタブ付き command modal を開き、bridge background 起動、pet asset 生成、Core2 upload、Codex relay、display settings、sample replay を任意パラメータで実行する。Dashboard の `実機反映` では表示設定の送信eventIdと実機heartbeatの `lastEventId` / display値を比較し、`未送信`、`反映待ち`、`反映済み`、`差分あり` を表示します。コマンド実行APIは localhost からの allowlist 実行だけを許可します。
