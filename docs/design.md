...
- hatch-pet asset がある場合は素材 frame と bounce を使う。Core2 では scale `1..8` ごとの高解像度 frame を生成しておき、`petScale=1..32` の範囲で最寄りの frame を選び、必要に応じて追加拡大して描く。未生成時は fallback avatar で色、blink、bounce、tail の animation を変える。
...
- hatch-pet asset は素材そのものを変形しすぎず、mood marker と小さな overlay で表情差分を出す。fallback avatar は目と口の形を変えて同じ mood を表現する。
