...
- pet surface は off-screen Sprite で合成してから画面へ転送する。animation tick は pet surface のみ更新し、Answer / Choice / footer などの本文領域を毎フレーム塗り直さない。
...
- pet animation FPS は Display tab から `4..20fps` で実機へ送信し、既定 `12fps` で固定 320ms 更新より滑らかに表示する。
...
- Display tab は pet display area、UI text size、body text size を `1..8` slider、animation FPS を `4..20` slider で調整する。
- M5Stack 表示プレビューは 320x240 の simulated display として、pet surface、Answer / Choice / Notification の本文、footer button label の表示密度、animation FPS 設定を送信前に確認できるようにする。
