...
- Codex App Server adapter smoke、runtime probe、adapter review、long-run source gate、signing readiness check、signed installer pipeline を追加済み。
...
実機確認は Core2 target の upload、Wi-Fi 接続、pairing、A button reply、Codex relay answer と、その後のユーザー目視確認までです。GRAY 実機と GRAY IMU は対象外です。実 Codex App Server 接続は thread / turn 作成まで確認済みです。長時間 soak、実署名 MSI / MSIX が未確認のため `S+` は付けません。ただし platform runtime gate は simulator / mock device / telemetry / adapter / safety boundary を通しており、Core2 target の LAN 接続も確認できたため、beta prerelease として `A-` 以上は許容します。
