...
- Host Bridge Dashboard と `codex:display` から pet 背景、文字色、文字背景を RGBA で送信し、M5Stack 表示へ反映できる。
...
- Dashboard の `M5Stack 表示プレビュー` から `petScale`、`uiTextScale`、`bodyTextScale` を `1..8`、`animationFps` を `4..20`、`motionStepMs` を `120..800`、RGBA、beep を送ると、M5Stack の pet 表示面積、テキストサイズ、描画更新上限、pose 切替頻度、色、通知音が次回描画または次回 answer から変わる。
