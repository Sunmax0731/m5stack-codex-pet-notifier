...
- Codex App Server public interface 連携に向け、`thread/start` と `turn/start` の JSON-RPC message builder、transport safety gate、adapter review を持つ。
...
- 署名付き MSI / MSIX へ進めるため、WiX MSI template、MSIX manifest template、Windows SDK / WiX / 署名用環境変数の readiness check を持つ。
...
- GRAY 実機と GRAY IMU が対象外であること、長時間運用 / 署名付き MSI / MSIX / 実 Codex App Server 接続が formal release 前の実運用確認対象であることを `docs/manual-test.md` と release notes に明記する。
