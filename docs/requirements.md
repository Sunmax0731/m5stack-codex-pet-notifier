...
- Host Bridge Dashboard から pet animation FPS を `4..20fps` の範囲で動的に変更でき、既定は `12fps` とする。
...
- Dashboard の `Display` tab から `petScale`、`uiTextScale`、`bodyTextScale` を `1..8`、`animationFps` を `4..20` の範囲で送ると、M5Stack の pet 表示面積、テキストサイズ、アニメーション速度が次回描画から変わる。
