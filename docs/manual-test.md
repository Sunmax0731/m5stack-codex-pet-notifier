...
| GUI-10 | `M5Stack 表示プレビュー` で pet display area、UI text size、body text size を `1..8`、render FPS を `4..20`、motion step を `120..800`、screen / pet / text / border RGBA、pet X/Y offset、beep、反映確認表示を変更する。`変更を自動送信` がonなら1秒程度待ち、offなら `表示設定を送信` を押す。色は各項目の RGBA picker で色と透明度を同時に操作し、swatch と `#hex / alpha` 表示を確認する | outbound に `display.settings_updated` が出る。payload に `screenBackgroundRgba`、`petBackgroundRgba`、`textColorRgba`、`textBackgroundRgba`、`petOffsetX`、`petOffsetY`、`textBorderEnabled`、`textBorderRgba`、`beepOnAnswer`、`visualProbe` が入る。10秒程度待つと inbound の `device.heartbeat` details に `display.applyCount`、`display.lastEventId`、送信した display 値が出る。`visualProbe=true` の場合は Core2 に短時間の反映確認画面が出る。古い bridge process では fallback の `pet.updated` でも可 | `dashboard:smoke` 済み。実機目視はユーザー手動 |
