...
| C2-14 | `codex:watch --once` で UTF-8 file 内容を送る | Core2 が `Answer` 画面へ遷移し、summary と file 内容を表示する | 未完了。event 送信と本文表示は確認したが、ユーザー提供画像は `Idle` 画面のため期待結果未達 |
...
- File watch evidence: `codex:watch --file dist\codex-answer.txt --once` で `Codex file watch answer` を送信し、Core2 画面へ本文が届いたことは確認済み。ただしユーザー提供画像は `Idle` 画面で、期待する `Answer` 画面遷移は未確認。
