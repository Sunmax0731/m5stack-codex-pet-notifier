...
- 署名付き MSI / MSIX の作業では `installer/wix/`、`installer/msix/`、`tools/windows-signing-check.mjs` を同時に確認し、証明書や PFX password は commit しない。
- Codex App 連携を変更する場合は public interface の `codex app-server` adapter を優先し、非公開 API scraping を追加しない。既存の session JSONL / hook relay は fallback adapter として残す。
...
`v0.2.0-beta.1` は Host Bridge、Dashboard、Codex relay、Codex app-server adapter preparation、M5Stack Core2 firmware、pet mood / gesture interaction、Windows user-local installer、署名付き MSI / MSIX 準備を含む beta prerelease です。GRAY 実機と GRAY IMU は対象外です。stable release ではないため、長時間運用、実 Codex App Server 接続、実署名、ユーザー環境での installer 実行確認は手動テストへ残します。
